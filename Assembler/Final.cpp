...
public:
    ...
    IDEX(int dpc, int jpc, int immm, Controller cww, int rs1, int rs2, string func3s, string func7s, string rdl,
         bool valid_flag, bool stall_flag) : DPC(dpc), JPC(jpc), imm(immm), RS1(rs1), RS2(rs2), func3(func3s),
                                             func7(func7s), Rdl(rdl), valid(valid_flag), stall(stall_flag)
    {
        cw = cww;
    }
};
...
public:
    ...
    EXMO()
    {
        DPC = BPC = JPC = RS2 = ALUOut = 0;
        Rdl = "00000";
        cw = Controller();
        valid = false, stall = false;
    }
    EXMO(int dpc, int bpc, int jpc, Controller cww, int aluout, int rs2, string rdl, bool valid_flag, bool stall_flag) : DPC(dpc), BPC(bpc), JPC(jpc),
...
public:
    ...
    MOWB(int dpc, int bpc, int jpc, Controller cww, int ldout, int aluout, string rdl, bool valid_flag, bool stall_flag) : DPC(dpc), BPC(bpc), JPC(jpc),
...
{
    if (Branch_stall_count)
    {
        Branch_stall_count--;
        cout << "BSCnt : " << Branch_stall_count << endl;
        ifid.valid = false;
        if (Branch_stall_count != 0)
            return;
        ifid.valid = true;
    }
...
void instruction_decode()
{
    ...
    idex.valid = true;
    ifid.valid = false;
    if (opcode == "1100011" || opcode == "1101111" || opcode == "1100111")
    {
        Branch_stall_count = 4;
    }
}
...
void Writeback()
{
    ...
    if (mowb.cw.branch)
        ...
        TPC = NPC;
    PC = TPC;
    cout << "BPC : " << mowb.BPC << endl;
    cout << "JPC : " << mowb.JPC << endl;
